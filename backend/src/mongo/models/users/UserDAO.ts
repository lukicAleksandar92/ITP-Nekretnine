...
class UserDAO{
    ...
    async login(korisnicko_ime: string, lozinka: string): Promise<User | null>{
        let user = await this.userModel.findOne({kor_ime: korisnicko_ime, lozinka: lozinka});
    ...
    async logUserLogin(kor_ime: string){
        return await this.userModel.updateOne({"kor_ime": kor_ime}, {$push: {"logins": new Date()}});
